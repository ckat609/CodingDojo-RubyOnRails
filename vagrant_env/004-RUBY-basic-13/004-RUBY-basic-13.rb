...
def printAll(aNumber)
    (1..aNumber).each{|x| puts x}
end
printAll(255)
...
def printEven(aNumber)
    (1..aNumber).each{|x| puts x if x.even?}
end
printEven(255)
...
def sumAll(aNumber)
    # puts "*"*50
    # (1..aNumber).each{|sum,x| puts "x: #{x} - sum: #{sum}"; sum+=x}
    # puts "*"*50
    sum = 0
    for i in 1..aNumber
        sum+=i
...
def printArray(aList)
    aList.each {|aNumber| puts aNumber}
end
...
def getAvg(aList)
    aList.inject(0){|sum,x| sum+x}/aList.length
end
puts getAvg([-3, -5, -7])
puts getAvg([1,3,5,7,9,13])
...
def oddArray(aNumber)
    aList = []
    (1..aNumber).each{|x| aList.push(x) if x.odd?}
    return aList.to_s
end
puts oddArray(255)
...
def greaterThan(aList, aNumber)
    sum=0
    aList.each{|x| sum+=1 if x > aNumber}
    return sum
end
puts greaterThan([1, 3, 5, 7], 3)
puts greaterThan([1, 3, 5, 7, 4, 9, 2, 6], 4)
...
def squareValues(aList)
    aList.map!{|x| x*x}
    return aList.to_s
end
puts squareValues([1, 2, 3, 4, 5, 6, 7, 8])
puts squareValues([1, 3, 5, 7, 4, 9, 2, 6])
...
def toZero(aList)
    aList.map!{|x| x < 0?0:x}.to_s
end
puts toZero([1, 5, 10, -2])
...
def minMaxAvg(aList)
    aHash = {
        'min': aList.min,
        'max': aList.max,
        'avg': aList.inject(0){|sum, x| sum+=x}/aList.length
    }
    return aHash
end
